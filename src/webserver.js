...
export default class WebServer {
    ...
    handleRedirect(request, response, contentType) {
        ...
        const domainKey = Object.keys(domainMap).find(key => request.url.startsWith(key));
        ...
        response.writeHead(302, { "Content-Type": contentType, Location: gdriveUrl });
        response.end();
    }
    ...
    constructFilePath(url) {
        ...
        const ignoreLastPeriod = formats.some(format => filePath.toLowerCase().includes(format));
