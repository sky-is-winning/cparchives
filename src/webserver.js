...
const MIME_TYPES = ["text/html", "text/css"];
...
export default class WebServer {
  ...
  async handleRequest(request, response) {
    ...
    if (
      ...
      request.url.startsWith("/media1/") ||
      request.url.startsWith("/swf.cpcheats.info/")
    ) {
  ...
  handleRedirect(request, response, contentType) {
    ...
    const domainKey = Object.keys(domainMap).find((key) =>
      request.url.startsWith(key),
    );
    const url = request.url.split(domainKey)[1];
    const gdriveUrl = this.getFileURLFromGDrive(url, domainMap[domainKey]);
  ...
  constructFilePath(url) {
    ...
    const ignoreLastPeriod = formats.some((format) =>
      filePath.toLowerCase().includes(format),
    );
  ...
  getFileURLFromGDrive(
    filePath,
    originalDomain = "archives.clubpenguinwiki.info",
  ) {
    ...
    for (let i = 0; i < fileArr.length; i++) {
      ...
      if (!jsonObject)
        return console.log(
          "Error: " + filePath + " not found in file structure",
        );
  ...
  getDriveURLFromRedLink(redLink) {
    ...
    return this.getFileURLFromGDrive(
      `/static/images/archives/${md5hash[0]}/${md5hash[0]}${md5hash[1]}/${fileName}`,
    );
  }
}
