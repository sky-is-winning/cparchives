...
export default class GenerateTemplate {
  ...
  generateNavbox(wt) {
    ...
    for (let list of Object.keys(boxData).filter((key) =>
      key.startsWith("list"),
    )) {
      let id = parseInt(list.replace("list", ""));
      navbox += `<tr style="height:2px;"><td></td></tr>`;
      navbox += `<td class="navbox-group accent" style=";;">${
        boxData["group" + id]
      }</td>`;
      let listContent = boxData[list];
      navbox += `<td style="text-align:left;border-left-width:2px;border-left-style:solid;width:100%;padding:0px;;;" class="navbox-list navbox-${
        id % 2 == 0 ? "even" : "odd"
      }"><div style="padding:0em 0.25em">${listContent}</div></td>`;
    }
  ...
  async generateTemplate(template) {
    ...
    let wikitext;
    try {
      wikitext = await fs.readFile(
        `./data/templates/${templateName}.wikitext`,
        "utf-8",
      );
    } catch (error) {
